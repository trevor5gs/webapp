...
class TextRegion extends Component {
  ...
  handleRegionClick(e) {
    ...
    // Relative links get sent to routeActions.push (usernames, raw links, hashtags)
    if (href && href[0] === '/') {
      e.preventDefault()
    ...
    // for. Once we change this back to an `<a> element we can rip this out.
      ...
      e.preventDefault()
      return dispatch(routeActions.push(dataset.href.replace(/^\/search/, '/find')))

    // Treat non links within grid layouts as a push to it's detail path
    } else if (isGridLayout && postDetailPath && nodeName !== 'A') {
      e.preventDefault()
      return dispatch(routeActions.push(postDetailPath))
    }
